// ============================
// 📄 routes/dbzDB/personajes/leer.js
// ============================
"use strict";
...
router.get("/leer", (req, res) => {
  ...
  connection.query(sql, (error, results) => {
    ...
    if (error) {
      ...
      return res
        .status(500)
        .json({ success: false, mensaje: "Error al leer personajes DBZ" });
    }

    // ✅ Convertimos el BLOB a Base64
    const personajes = results.map(p => {
      let base64 = null;
      if (p.image_blob) {
        try {
          ...
          console.warn(`⚠️ No se pudo convertir el BLOB del personaje ${p.id_api}`);
        }
      }
