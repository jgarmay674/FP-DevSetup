// ============================
// 📄 routes/dbzDB/transformaciones/actualizarImagen.js
// ============================
"use strict";
...
router.put("/actualizarImagen/:id_api",
    ...
    (req, res) => {
        ...
        const sql = "UPDATE transformaciones SET image_blob = ? WHERE id_api = ?";
        ...
        connection.query(sql, [buffer, id_api], (error, results) => {
            ...
            if (results.affectedRows === 0) {
                return res.status(404).json({ success: false, mensaje: "Transformación no encontrada" });
            }

            res.json({ success: true, mensaje: `🖼️ Imagen de la transformación ${id_api} actualizada` });
        });
    }
);
