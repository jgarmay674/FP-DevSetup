#!/usr/bin/env bash
# =============================================================================
#  FP-DevSetup / windows/setup.sh
#  Entorno de desarrollo para Formación Profesional — DAW
#  Plataforma: Windows con WSL2 + Ubuntu
#  Curso 2026-2027
#
#  REQUISITOS PREVIOS (manuales en Windows antes de ejecutar este script):
#  1. WSL2 + Ubuntu:    wsl --install -d Ubuntu   (PowerShell como admin)
#  2. Docker Desktop:   https://www.docker.com/products/docker-desktop
#     → Settings → Resources → WSL Integration → activar Ubuntu
#  3. VS Code:          https://code.visualstudio.com
#     → marcar "Add to PATH" · instalar extensión WSL
# =============================================================================
...
echo -e "  ${CYAN}${BOLD}║   Plataforma : Windows WSL2 + Ubuntu                         ║${RESET}"
echo -e "  ${CYAN}${BOLD}║                                                              ║${RESET}"
...
log_warn "Verificando prerequisitos de Windows/WSL2..."
PREREQS_OK=true

if ! command -v docker &>/dev/null; then
  log_error "Docker no está disponible. ¿Has instalado Docker Desktop y activado WSL2 integration?"
  PREREQS_OK=false
fi

if ! command -v code &>/dev/null; then
  log_warn "VS Code no está en el PATH de WSL2."
  log_warn "Asegúrate de instalar VS Code en Windows con 'Add to PATH' y reiniciar WSL2."
fi

if [[ "$PREREQS_OK" == "false" ]]; then
  echo ""
  log_error "Instala los prerequisitos manualmente antes de continuar (ver comentarios del script)."
  exit 1
fi
...
run_step "01" "Sistema base" \
  "01_sistema.sh" \
  "apt en WSL2 · Git · GitHub CLI · SSH (Docker vía Docker Desktop en Windows)."
...
run_step "04" "Aliases de terminal" \
  "04_aliases.sh" \
  "Aliases para Git, Docker y utilidades en bash/zsh."
...
echo -e "  1. ${YELLOW}Docker Desktop arrancado${RESET} en Windows (WSL Integration activa)"
echo -e "  2. Autentícate en GitHub:  ${CYAN}gh auth login${RESET}"
echo -e "     ${YELLOW}(WSL: si no abre navegador → https://github.com/login/device)${RESET}"
